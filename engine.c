...
static int bounded_buffer_init(bounded_buffer_t *buf) {
    ...
    if ((rc = pthread_cond_init(&buf->not_empty, NULL)) != 0) { pthread_mutex_destroy(&buf->mutex); return rc; }
...
void *logging_thread(void *arg) {
    ...
    while (bounded_buffer_pop(buf, &item) == 0) {
        ...
        if (write(fd, item.data, item.length) < 0) perror("logging_thread: write");
        close(fd);
    }
    return NULL;
}
...
static pid_t launch_container(supervisor_ctx_t *ctx, const control_request_t *req) {
    ...
    if (pid < 0) {
        perror("clone"); free(cfg); close(pipefd[0]); close(pipefd[1]); return -1;
    }
    ...
    usleep(200000);  /* Let child exec before registering */
    
    if (ctx->monitor_fd >= 0) {
        if (register_with_monitor(ctx->monitor_fd, req->container_id, pid, req->soft_limit_bytes, req->hard_limit_bytes) != 0)
...
static void handle_client(supervisor_ctx_t *ctx, int client_fd) {
    ...
    if (req.kind == CMD_START || req.kind == CMD_RUN) {
        ...
        char buf[4096];
        int  off = 0;
        off += snprintf(buf + off, sizeof(buf) - off, "%-16s %-8s %-10s %-12s %-12s\n", "ID", "PID", "STATE", "SOFT(MiB)", "HARD(MiB)");
        
        pthread_mutex_lock(&ctx->metadata_lock);
        container_record_t *c = ctx->containers;
        while (c && off < (int)sizeof(buf) - 80) {
            off += snprintf(buf + off, sizeof(buf) - off, "%-16s %-8d %-10s %-12lu %-12lu\n",
        ...
        resp.status = 0;
        snprintf(resp.message, sizeof(resp.message), "%.*s", (int)sizeof(resp.message) - 1, buf);
        send(client_fd, &resp, sizeof(resp), 0);
        ...
        kill(pid, SIGTERM);
        resp.status = 0;
        snprintf(resp.message, sizeof(resp.message), "sent SIGTERM to '%s' pid=%d", req.container_id, pid);
        send(client_fd, &resp, sizeof(resp), 0);
    } else {
        resp.status = -1;
...
static int run_supervisor(const char *rootfs) {
    ...
    if (ctx.monitor_fd < 0) fprintf(stderr, "Warning: /dev/container_monitor not available\n");
